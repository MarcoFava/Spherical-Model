...
int main(int argc, char* argv[])
{
    ...
    /*################################ SAVE ALL PARAMETERS ################################*/
    // the parameters are saved in this order: dim, L, T_i, T_f, n_T_steps, theta, n_MC, burnin, out_num.
    ...
    int n_T_steps = (int)parameters[4];     // number of temperature steps between T_i and T_f
    double theta = parameters[5];           // metropolis' parameter, in radians
    int n_MC = (int)parameters[6];          // number of Markov Chain Monte Carlo cycles
    int burnin = (int)parameters[7];        // burn-in time
    out_num = (int)parameters[8];           // extern parameter to change the name of the output file
